...
describe("ConferenceTicketItem", () => {
  ...
  it('should clamp initial invalid quality between MINIMUM_QUALITY and MAXIMUM_QUALITY', () => {
    const item = new ConferenceTicketItem('Conference Ticket', 10, MINIMUM_QUALITY - 10);
    item.updateQuality();
    expect(item.quality).toBe(MINIMUM_QUALITY);

    const item2 = new ConferenceTicketItem('Conference Ticket', 10, MAXIMUM_QUALITY + 10);
    item2.updateQuality();
